...
聊天摘要应像律师给客户提供建议——结论先行，用通俗语言说，语气自然。先让用户知道"这个数据传输场景的主要风险和应对方向"，再讲最关键的理由。

底线要求：
- 必须包含结论方向（"需要做 TIA""触发 DPIA""SCC 可覆盖"等）。
- 必须说明已生成正式报告。
- 不得把完整报告内容塞进聊天消息。
...
首轮研究任务必须生成一份最终 Markdown 报告。正式报告结构以 `assets/eu-data-transfer-tia-dpia-report-template.md` 为唯一真源。本文件只规定输出层级、GDPR / EDPB / 判例引用方式、风险标签和人工复核边界，不再单独维护报告章节。

生成或更新正式报告前必须先读取 `assets/eu-data-transfer-tia-dpia-report-template.md`，并按模板中的标题、章节、表格和条件触发段落填充内容。不得用本文件的聊天摘要结构替代正式报告，也不得恢复旧版的“检索范围与来源说明”“引用与证据”“结论限制与人工复核触发”等内部说明式章节。
